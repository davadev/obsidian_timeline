...
| Name events too short to label | on | Draws the name beside the bar, with a short leader pointing at it, for point events and for spans too narrow to hold their own label at the current zoom. Only the event's own width counts, so a long bar scrolled halfway off screen keeps its sliding label instead of gaining a name, and an event is named only while it is fully in view. Names are packed into the free part of the same lane, never overlap anything, and keep the same text as you scroll. Per-block: `shortEventLabels: false`. |
