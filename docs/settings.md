...
| Keep event labels in view | on | Slides each event label along its bar while scrolling, so a span running off both edges of the screen still shows its name. Off pins the label to the bar's start. Per-block: `stickyLabels: false`. **Applies to inline blocks only since 0.11** — the Timeline view's windowed chart always slides labels; see [architecture](architecture.md#unfinished-and-why-it-matters). |
